...
cp ../monkeypox-build/data/metadata_region.tsv ../monkeypox-build/results/beauti/
...
awk -F"\t" 'OFS="\t" {$1=$5"_"$4"_"$3; print}' metadata_region.tsv > meta.tsv
...
cat align.fasta | awk '/>MPXV_USA_2021_MD_NorthAmerica_2021-11-24/ {getline; while(!/>/) {getline}} 1' > fixed_region.fasta
